...
def test_interprets_eligibility_question_with_lead_time():
    ...
    assert result.conditions == {"lead_time": "2일"}
    assert "문서 기준" in result.canonical_question
    assert "2일" in result.canonical_question
...
def test_interprets_annual_leave_eligibility_as_deadline_check():
    ...
    assert "날짜 계산은 만들지 말라" in result.canonical_question
    assert "거절" in result.canonical_question
...
def test_interprets_four_days_later_annual_leave_question_for_retrieval():
    result = interpret_question("4일뒤에 연차 쓰려고 하는데 가능할까요?")

    assert result.retrieval_question == "연차 유급휴가 신청 기한 최소 영업일 전"
    assert result.intent == ELIGIBILITY_CHECK
    assert result.conditions == {"lead_time": "4일"}
    assert "연차를 4일 뒤에 사용" in result.canonical_question
...
def test_interprets_general_question_when_no_pattern_matches():
    result = interpret_question("회사 휴가 규정에 대해 알려주세요")

    assert result.intent == GENERAL_QA
    assert result.conditions == {}
    assert result.canonical_question == "회사 휴가 규정에 대해 알려주세요"
